...
export default function ResultsPage() {
  ...
  const handleShareProfile = async () => {
    try {
      setCopying(true);
      const shareText = `Check out my IntervueAI performance! I scored ${score}/100 in my latest AI-powered interview simulation.`;
      const shareUrl = window.location.origin;
      
      if (navigator.share) {
        await navigator.share({
          title: 'My IntervueAI Report',
          text: shareText,
          url: shareUrl,
        });
      } else {
        await navigator.clipboard.writeText(`${shareText}\n${shareUrl}`);
        alert('Profile link and summary copied to clipboard!');
      }
    } catch (err) {
            ...
            <div className="flex-1 relative z-10">
              ...
              <div className="flex flex-wrap gap-4">
                <button 
                  ...
                  className="bg-navy-900 text-white px-6 py-2.5 rounded-xl font-semibold hover:bg-navy-800 transition shadow-sm active:scale-95"
                >
                ...
                <button 
                  ...
                  disabled={copying}
                  className="bg-white border border-slate-200 text-navy-900 px-6 py-2.5 rounded-xl font-semibold hover:bg-slate-50 transition shadow-sm active:scale-95 disabled:opacity-50"
                >
                  {copying ? 'Sharing...' : 'Share Profile'}
                </button>
              </div>
            </div>
          </div>
